...
class Ajax {
	...
	public function save_products() {
		...
		$this->fields   = BWCEdit_Data()->get_fields_for_parse_product();
		$handle_product = Handle_Product::instance();
	...
	public function search_products() {
		...
		foreach ( $ids as $id ) {
			...
			if ( $managing_stock && ! empty( $_GET['display_stock'] ) ) {
				...
				$formatted_name .= ' &ndash; ' . sprintf( __( 'Stock: %d', 'bulky-bulk-edit-products-for-woo' ), wc_format_stock_quantity_for_display( $stock_amount, $product_object ) );
			}
	...
	private function product_duplicate( $product ) {
		...
		$duplicate->set_name( sprintf( esc_html__( '%s (Copy)', 'bulky-bulk-edit-products-for-woo' ), $duplicate->get_name() ) );

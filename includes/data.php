	...
	public function define_columns_type() {
		...
		$columns = [
			...
			'post_date' => [
				'type'    => 'calendar',
				'width'   => 120,
				'title'   => esc_html__( 'Publish date', 'bulky-bulk-edit-products-for-woo' ),
...
//			'author' => [ 'type' => 'dropdown', 'width' => 100, 'title' => esc_html__( 'Author', 'bulky-bulk-edit-products-for-woo' ), 'source' => $users, ],
			...
			'shipping_class' => [
				...
				'source'    => array_merge( [ [ 'id' => '0', 'name' => esc_html__( 'No shipping class', 'bulky-bulk-edit-products-for-woo' ) ] ], $shipping_class ),
				'subSource' => array_merge( [ [ 'id' => '0', 'name' => esc_html__( 'Same as parent', 'bulky-bulk-edit-products-for-woo' ) ] ], $shipping_class ),
				'filter'    => 'sourceForVariation'
			],
		];
		...
		if ( empty( $user_product_column_width ) ) {
	...
	public function get_settings() {
		if ( ! $this->settings ) {
			...
			$user_option = get_user_meta( get_current_user_id(), 'vi_wbe_settings', true );
			$user_option = empty( $user_option ) ? [] : $user_option;

			$this->settings = empty( $user_option ) ? wp_parse_args( get_option( 'vi_wbe_settings', [] ), $default_settings ) : wp_parse_args( $user_option, $default_settings );
		}
